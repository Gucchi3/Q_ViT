"""
module.py
Q_EfficientFormer V2 量子化モジュール
（i_vit/quant_module.py と同一内容。他モデルのモジュールを遠隔参照しないようここに置く）
"""
...
class QuantLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True,
                 weight_bit=8, bias_bit=32, per_channel=True, quant_mode='symmetric'):
...
class TerLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True,
                 weight_bit=2, bias_bit=32, per_channel=True,
                 quant_mode='symmetric', use_layernorm=False):
...
class QuantAct(nn.Module):
    def __init__(self, activation_bit=8, act_range_momentum=0.95,
                 running_stat=True, per_channel=False, quant_mode="symmetric"):
...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size,
                 ...
                 weight_bit=8, bias_bit=32, quant_mode="symmetric", per_channel=True):
...
class QuantBNConv2d(nn.Module):
    """
    BN を Conv に吸収する量子化畳み込み（QAT 用）

    訓練時のフロー:
      1. float Conv を no_grad で実行し、BatchNorm の running_mean/var を EMA 更新
      2. running stats を使って BN パラメータ (γ, β, μ, σ) を Conv に吸収
             W_fold  = W × γ / sqrt(var + ε)       (per output channel)
             b_fold  = β − μ × γ / sqrt(var + ε)
      3. W_fold と b_fold を疑似量子化（STE）
             → 勾配は W, γ, β へ正しく伝播
      4. 整数 Conv を実行
    """
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(
            torch.empty(out_channels, in_channels // groups, *ks))
        ...
        self.register_buffer('weight_integer',      torch.zeros_like(self.weight))
        self.register_buffer('bias_integer',        torch.zeros(out_channels))
        if quant_mode == "symmetric":
            self.weight_function = SymmetricQuantFunction.apply
        else:
    ...
    def _fold_bn(self):
        ...
        γ   = self.bn.weight
        β   = self.bn.bias
        μ   = self.bn.running_mean
        σ2  = self.bn.running_var
        σ   = (σ2 + self.bn.eps).sqrt()
        s   = γ / σ
        return W * s.view(-1, 1, 1, 1), β - μ * s

    def forward(self, x, pre_act_scaling_factor):
        if self.training:
            with torch.no_grad():
                y_fp = F.conv2d(
                    x, self.weight.detach(), None,
                    self.stride, self.padding, self.dilation, self.groups)
                _ = self.bn(y_fp)
        W_fold, b_fold = self._fold_bn()
        with torch.no_grad():
            v = W_fold.detach().reshape(W_fold.shape[0], -1)
            self.conv_scaling_factor = symmetric_linear_quantization_params(
                self.weight_bit, v.min(1).values, v.max(1).values)
        self.weight_integer = self.weight_function(
            W_fold, self.weight_bit, self.conv_scaling_factor, True)
        bias_scaling_factor = self.conv_scaling_factor * pre_act_scaling_factor
        self.bias_integer = self.weight_function(
            b_fold, self.bias_bit, bias_scaling_factor, True)
        x_int = x / pre_act_scaling_factor
        correct_output_scale = bias_scaling_factor.view(1, -1, 1, 1)
        return (
            F.conv2d(x_int, self.weight_integer, self.bias_integer,
                     self.stride, self.padding, self.dilation, self.groups)
