...
class TerLinear(nn.Linear):
    """三値量子化 Linear (weight_bit=2, ±1/0 のみ)"""
    def __init__(self, in_features, out_features, bias=True,
...
class QuantBNConv2d(nn.Module):
    ...
    def __repr__(self):
        return (f"QuantBNConv2d({self.in_channels}, {self.out_channels}, "
                f"kernel_size={self.kernel_size}, stride={self.stride}, "
                f"padding={self.padding}, groups={self.groups}, "
                f"weight_bit={self.weight_bit})")

    def _fold_bn(self):
        W, γ, β = self.weight, self.bn.weight, self.bn.bias
        μ, σ2   = self.bn.running_mean, self.bn.running_var
        s = γ / (σ2 + self.bn.eps).sqrt()
        return W * s.view(-1, 1, 1, 1), β - μ * s

    def forward(self, x, pre_act_scaling_factor):
        if self.training:
            with torch.no_grad():
                y_fp = F.conv2d(x, self.weight.detach(), None,
