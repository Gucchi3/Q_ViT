...
try:
    ...
    from .utils import trunc_normal_, to_2tuple
except ImportError:
    # python swin.py として直接実行する場合
    import sys, os
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", ".."))
    from model.swin.quant_module import (QuantLinear, QuantAct, QuantConv2d, IntLayerNorm,
                                          IntSoftmax, IntGELU, QuantMatMul, TerLinear)
    from model.swin.utils import trunc_normal_, to_2tuple
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.qact1 = QuantAct()
        self.reduction = QuantLinear(4 * dim, 2 * dim, bias=False)
        self.qact2 = QuantAct()
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                act_layer=IntGELU, norm_layer=norm_layer)
    ...
    def forward(self, x, act_scaling_factor):
        for blk in self.blocks:
            if not torch.jit.is_scripting() and self.use_checkpoint:
                x, act_scaling_factor = checkpoint.checkpoint(blk, x, act_scaling_factor)
            else:
